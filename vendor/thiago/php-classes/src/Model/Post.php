...
class Post extends Model {
    ...
    public function save() {
        $sql = new Sql();
        $results = $sql->select("CALL sp_posts_save(:idpost, :titulo, :autor, :texto, :post_active, :id_user)", array(
    ...
    public static function getCarouselSingle() {
        $sql = new Sql();
        return $sql->select("SELECT * FROM tb_posts ORDER BY idpost DESC LIMIT 1");
    }

    public static function getCarousel() {
        $sql = new Sql();
        return $sql->select("SELECT * FROM tb_posts WHERE idpost NOT IN(SELECT MAX(idpost) from tb_posts) ORDER BY idpost DESC Limit 2");
    }
